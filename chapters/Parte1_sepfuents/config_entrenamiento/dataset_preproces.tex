...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textit{mix\_magnitude}
        \item \textit{source\_magnitudes}
    \end{itemize}
    
    Y para pérdidas que piden fase:
    \begin{itemize}
        \item \textit{mixture\_phase}
        \item \textit{source\_phase}
    \end{itemize}
    ...
    \end{itemize}
    División 60-40\footnote{60\% train 40\% validation}.

    \item \textbf{6. Normalización/canales:}
    El sistema se configuró para un canal de audio, lo que reduce el coste computacional y se recomienda en la documentación del modelo\cite{rafii2018overview}. También simplifica el proceso de comparación entre fuentes.
